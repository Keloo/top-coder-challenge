...
# Check if run_eval.sh exists
if [ ! -f "run_eval.sh" ]; then
    echo "❌ Error: run_eval.sh not found!"
    echo "Please create a run_eval.sh script that takes three parameters:"
    echo "  ./run_eval.sh <trip_duration_days> <miles_traveled> <total_receipts_amount>"
    echo "  and outputs the reimbursement amount"
    exit 1
fi

# Make run_eval.sh executable
chmod +x run_eval.sh
...
for ((i=0; i<num_cases; i++)); do
    ...
    if script_output=$(./run_eval.sh "$trip_duration" "$miles_traveled" "$receipts_amount" 2>/dev/null); then
        ...
        error_msg=$(./run_eval.sh "$trip_duration" "$miles_traveled" "$receipts_amount" 2>&1 >/dev/null | tr -d '\n')
...
echo "  2. Ensure your run_eval.sh outputs only a number"
